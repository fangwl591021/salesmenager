...
window.showToast = function(msg, isError = false) {
  ...
  t.className = `fixed top-14 left-1/2 -translate-x-1/2 px-5 py-3 rounded-full text-[14px] shadow-lg transition-all font-bold flex items-center gap-2 w-max max-w-[90vw] ${isError ? 'bg-red-500 text-white border-red-600' : 'bg-gray-800 text-white border-gray-700'} opacity-100`;
...
function setButtonLoading(btnId, isLoading, originalText = '') {
    ...
    if (isLoading) {
        ...
        btn.innerHTML = '<i data-lucide="loader-2" class="w-5 h-5 animate-spin align-middle inline-block"></i>';
  ...
  try {
    ...
    if (shareCardId) {
        ...
        try {
            ...
            window.showToast('名片轉發失敗: ' + e.message, true);
        }
        return; 
    }
...
async function loadCardContacts() { 
    ...
    if (cachedDataString) {
        ...
        container.innerHTML = '<div class="text-center py-10"><i data-lucide="loader-2" class="w-8 h-8 text-blue-500 animate-spin mx-auto"></i></div>';
...
window.renderCardPage = function(isReset = false) {
    const container = document.getElementById('admin-card-list-container');
    if (!container) return;
    if (filteredCards.length === 0) { container.innerHTML = '<div class="text-center py-16 text-gray-500 font-bold">查無名片</div>'; return; }
    ...
    let html = pageData.map(c => {
        ...
        return `
        <div onclick="if(typeof window.openCardDetailByRowId === 'function') window.openCardDetailByRowId('${c.rowId}')" class="p-5 bg-white flex items-center gap-4 rounded-[2rem] shadow-sm mb-3 cursor-pointer active:scale-[0.98] transition-transform border border-gray-100">
          <div class="w-[56px] h-[56px] shrink-0 rounded-2xl overflow-hidden bg-gray-50 flex items-center justify-center border border-gray-100">
            ${c['名片圖檔'] && c['名片圖檔'] !== '圖片儲存失敗' && c['名片圖檔'] !== '無圖檔' ? `<img src="${window.getDirectImageUrl(c['名片圖檔'])}" class="w-full h-full object-cover">` : `<i data-lucide="contact" class="text-gray-300 w-7 h-7"></i>`}
          </div>
          <div class="flex-1 overflow-hidden flex flex-col justify-center gap-1">
            <div class="flex items-center"><h4 class="text-[17px] font-extrabold text-gray-800 truncate">${c['姓名'] || '未知姓名'}</h4>${isAdmin ? claimBadge : ''}</div>
            <p class="text-[13px] text-gray-500 font-medium truncate">${c['公司名稱'] || c['職稱'] || '無資訊'}</p>
          </div>
        </div>
    `}).join('');
    ...
    if (currentPage * PAGE_LIMIT < filteredCards.length) {
        const wrapper = document.getElementById('card-list-wrapper');
        if (wrapper) {
            wrapper.insertAdjacentHTML('afterend', `<button id="${loadMoreBtnId}" onclick="if(typeof window.loadMoreCards === 'function') window.loadMoreCards()" class="w-full py-4 mt-2 bg-white text-gray-700 font-bold text-[15px] rounded-[2rem] shadow-sm active:scale-95 transition-transform flex justify-center items-center gap-1 border border-gray-100">顯示更多</button>`);
        }
    }
}
    ...
    try {
      ...
      if (isAdmin && statusEl) {
          if (card.userId || card['LINE ID'] || card['Line ID'] || card['lineId']) {
              ...
              statusEl.className = 'px-2.5 py-0.5 rounded-md text-[11px] bg-gray-50 text-gray-400 font-bold';
          }
      ...
      const flexMessage = {
          ...
          body: {
              ...
              contents: [
                  ...
                  { type: "text", text: "我已為您建立了數位商務名片。點擊下方按鈕即可認領名片、自由編輯內容，並啟用數位轉發功能！", size: "sm", color: "#6b7280", wrap: true, margin: "md" }
              ]
          },
          footer: {
...
window.confirmCrop = async function() { 
    ...
    if (currentCropTarget === 'ecard' || currentCropTarget === 'v2logo') {
      ...
      if(btn) { 
          btn.innerHTML = '<i data-lucide="loader-2" class="w-5 h-5 animate-spin"></i> 上傳中'; 
          if (typeof lucide !== 'undefined') lucide.createIcons({ root: btn });
...
window.saveToCloud = async function() {
  ...
  if (!payload.Name && !payload.CompanyName) { setButtonLoading('btn-save', false, '存入雲端 <i data-lucide="upload-cloud" class="w-[18px] h-[18px]"></i>'); isProcessing = false; return window.showToast("⚠️ 請輸入姓名或公司", true); }
  ...
  } catch(err) { console.error(err); } finally { setButtonLoading('btn-save', false, '存入雲端 <i data-lucide="upload-cloud" class="w-[18px] h-[18px]"></i>'); isProcessing = false; }
}
...
window.batchRegenerateECards = async function() {
    ...
    if (btn) { 
      btn.innerHTML = '<i data-lucide="loader-2" class="w-4 h-4 animate-spin inline-block align-middle mr-1"></i> 處理中...'; 
      if (typeof lucide !== 'undefined') lucide.createIcons({ root: btn });
      btn.classList.add('pointer-events-none', 'opacity-50'); 
    }
    let updatedCount = 0;
    try {
        ...
        localStorage.removeItem(CACHE_KEY_CONTACTS);
        loadCardContacts();
    } catch (err) { window.showToast("更新錯誤", true); } finally { 
      if (btn) { 
        btn.innerHTML = '<i data-lucide="refresh-cw" class="w-4 h-4 inline-block align-middle mr-1"></i> 批次重新讀取寫入所有名片按鈕'; 
        if (typeof lucide !== 'undefined') lucide.createIcons({ root: btn });
        btn.classList.remove('pointer-events-none', 'opacity-50'); 
      } 
    }
...
window.triggerFlexSharing = async (flexContents, altText) => {
    ...
    try {
        ...
        if (error.message && error.message.includes('not allowed')) {
            window.showToast("⚠️ 系統權限更新，即將重新登入", true);
            setTimeout(() => { liff.logout(); window.location.reload(); }, 1500); 
        } else {
            window.showToast('發送取消或發生錯誤', true);
        }
    }
};
